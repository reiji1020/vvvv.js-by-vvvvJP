...
define(function (require, exports) {
  ...
  return function (root_patch) {
    ...
    this.connect = function (success_callback) {
      if (VVVVContext.name !== "browser" || this.socket !== null) return;
      this.socket = new WebSocket(
        "ws://" + location.hostname + ":5001/vvvvjs-rt"
      );
      ...
      this.socket.onopen = function () {
        const msg = {
          app_root: location.pathname.substring(
            0,
        ...
        root_patch.resourcesPending--;
        if (success_callback) success_callback.call(root_patch);
      };
      ...
      this.processMessage = function (patchIdentifier) {
        ...
        let msg;
        if (!messageQueue[patchIdentifier]) return;
        if ((msg = messageQueue[patchIdentifier].shift()) !== undefined) {
          ...
          while (i--) {
            ...
            for (let pinname in node.pinValues) {
              ...
              while (j--) {
                p.nodeMap[node.node_id].outputPins[pinname].setValue(
                  j,
                  node.pinValues[pinname][j]
                );
              }
              p.nodeMap[node.node_id].outputPins[pinname].setSliceCount(
                node.pinValues[pinname].length
              );
            }
          }
        }
      ...
      this.socket.onmessage = function (str) {
        ...
        if (msg.nodes) {
          if (!messageQueue[msg.patch]) messageQueue[msg.patch] = [];
    ...
    this.sendPatchUpdate = function (patch, command) {
      ...
      const msg = {
        patch: VVVV.Helpers.prepareFilePath(patch.nodename, patch.parentPatch),
        command: command,
      };
