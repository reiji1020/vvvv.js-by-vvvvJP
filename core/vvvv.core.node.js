...
define(function (require, exports) {
  ...
  return function (id, nodename, parentPatch) {
    ...
    this.setupObject = function () {
      // had to put this into a method to allow Patch to "derive" from Node. Really have to understand this javascript prototype thing some day ...
      this.inputPins = {};
      this.outputPins = {};
      this.invisiblePins = {};
    ...
    if (parentPatch) this.parentPatch.nodeMap[id] = this;
    ...
    this.addInvisiblePin = function (pinname, value, type) {
      ...
      const pin = new Pin(
        pinname,
        VVVV.PinDirection.Configuration,
        value,
        this,
        type
      );
    ...
    this.getWidth = function () {
      ...
      if (this.width === 100 || this.width === 0) {
        if (this.isIOBox) ret = 60;
        else ret = Math.max(18, (this.label().length + 2) * 6);
      } else ret = this.width / 15;
    ...
    this.getHeight = function () {
      if (this.isIOBox && this.height === 100) return 18 * this.IOBoxRows();
      if (this.height === 100 || this.isSubpatch) return 18;
      else return Math.max(18, this.height / 15);
    };
    ...
    this.getUpstreamNodes = function () {
      ...
      _(this.inputPins).each(function (p) {
        if (p.links.length > 0) ret.push(p.links[0].fromPin.node);
      });
      return ret;
    };
        ...
        for (var j = 0; j < p.links.length; j++) {
          if (p.links[j].toPin.node.nodename === name)
            ...
            if (p.links[j].toPin.slavePin) {
              // enter subpatch
              ret = ret.concat(
                p.links[j].toPin.slavePin.node.findDownstreamNodes(name)
              );
            } else if (
              p.links[j].toPin.node.isIOBox &&
              p.links[j].toPin.node.IOBoxOutputPin().slavePin
            ) {
              // leave subpatch
              ret = ret.concat(
                p.links[j].toPin.node
                  .IOBoxOutputPin()
                  .slavePin.node.findDownstreamNodes(name)
              );
            } else
    ...
    this.hasNilInputs = function () {
      let result = false;
      _(this.inputPins).each(function (p) {
        if (p.getSliceCount() === 0 || p.values[0] === undefined) result = true;
      });
      return result;
    };
    ...
    this.isDirty = function () {
      ...
      for (pinname in this.inputPins) {
        if (this.inputPins[pinname].pinIsChanged()) return true;
      }
      for (pinname in this.invisiblePins) {
        if (this.invisiblePins[pinname].pinIsChanged()) return true;
      }
      return false;
    };
    ...
    this.registerInterfacePin = function () {
      ...
      if (this.isIOBox) {
        if (
          this.parentPatch.parentPatch &&
          this.invisiblePins["Descriptive Name"].getValue(0) !== ""
        ) {
          let pinname = this.invisiblePins["Descriptive Name"].getValue(0);
          this.IOBoxInputPin().connectionChangedHandlers[
            "subpatchpins"
          ] = function () {
            ...
            if (that.IOBoxOutputPin().links.length === 0) {
              if (!that.IOBoxOutputPin().slavePin) {
                ...
                if (pin === undefined) {
                  pin = that.parentPatch.addOutputPin(
                    pinname,
                    that.IOBoxOutputPin().values
                  );
                }
              ...
              } else if (that.IOBoxOutputPin().slavePin.pinname !== pinname) {
                // rename subpatch pin
                if (VVVV_ENV === "development")
                  console.log(
                    "renaming " +
                      that.IOBoxOutputPin().slavePin.pinname +
                      " to " +
                      pinname
                  );
                that.parentPatch.outputPins[pinname] =
                  that.parentPatch.outputPins[
                    that.IOBoxOutputPin().slavePin.pinname
                  ];
                that.parentPatch.removeOutputPin(
                  that.IOBoxOutputPin().slavePin.pinname
                );
          ...
          this.IOBoxOutputPin().connectionChangedHandlers[
            "subpatchpins"
          ] = function () {
            let i;
            ...
            if (that.IOBoxInputPin().links.length === 0) {
              if (!that.IOBoxInputPin().masterPin) {
                ...
                if (pin === undefined) {
                  ...
                  pin = that.parentPatch.addInputPin(
                    pinname,
                    that.IOBoxInputPin().values
                  );
                }
                ...
                if (
                  pin.unvalidated &&
                  VVVV.PinTypes[pin.typeName].primitive &&
                  !pin.isConnected()
                ) {
                  if (pin.typeName[0] === "V") {
              ...
              } else if (that.IOBoxInputPin().masterPin.pinname !== pinname) {
                // rename subpatch pin
                console.log(
                  "renaming " +
                    that.IOBoxInputPin().masterPin.pinname +
                    " to " +
                    pinname
                );
                that.parentPatch.inputPins[pinname] =
                  that.parentPatch.inputPins[
                    that.IOBoxInputPin().masterPin.pinname
                  ];
                that.parentPatch.removeInputPin(
                  that.IOBoxInputPin().masterPin.pinname
                );
    ...
    this.serialize = function () {
      ...
      if (this.shaderFile) {
        $node.attr(
          "filename",
          this.shaderFile
            .replace(".vvvvjs.fx", ".fx")
            .replace("%VVVV%/effects", "%VVVV%/lib/nodes/effects")
        );
      }
      if (this.isSubpatch) {
      ...
      if (this.isIOBox) $node.attr("componentmode", "InABox");
      else $node.attr("componentmode", "Hidden");

      var $bounds = $("<BOUNDS>");
      if (this.isIOBox) $bounds.attr("type", "Box");
      else $bounds.attr("type", "Node");
      ...
      _(this.inputPins).each(function (p) {
        ...
        $pin.attr("visible", "1");
        if (
          (!p.isConnected() || p.masterPin) &&
          VVVV.PinTypes[p.typeName].primitive &&
          that.defaultPinValues[p.pinname]
        ) {
          $pin.attr(
            "values",
            _(that.defaultPinValues[p.pinname])
              .map(function (v) {
    ...
    this.sharedRessources = function () {
      if (
        !VVVVContext.sharedRessourceStores[
          this.parentPatch.getPatchIdentifier() + "/" + this.id
        ]
      ) {
        let SharedRessourceStore = require("core/vvvv.core.shared_ressources");
        VVVVContext.sharedRessourceStores[
          this.parentPatch.getPatchIdentifier() + "/" + this.id
        ] = new SharedRessourceStore();
      }
      return VVVVContext.sharedRessourceStores[
        this.parentPatch.getPatchIdentifier() + "/" + this.id
      ];
    };
  };
