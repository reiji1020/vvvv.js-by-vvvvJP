...
define(function (require, exports) {
  ...
  return function (fromPin, toPin) {
    ...
    this.destroy = function () {
      ...
      this.fromPin.node.parentPatch.linkList.splice(
        this.fromPin.node.parentPatch.linkList.indexOf(this),
        1
      );

      this.toPin.disconnect();
      if (this.toPin.reset_on_disconnect) this.toPin.reset();
      else {
        ...
        while (i--) {
          pincmd[this.toPin.pinname].values[i] = this.toPin.values[
            i
          ].toString();
        }
        if (
          this.fromPin.node.parentPatch.editor &&
          !this.fromPin.node.parentPatch.disposing
        )
          this.fromPin.node.parentPatch.editor.update(
            this.fromPin.node.parentPatch,
            cmd
          );
      }
    };
