...
define(function (require, exports) {
  ...
  return function (pinname, direction, init_values, node, type) {
    ...
    this.getValue = function (i, binSize) {
      if (!binSize || binSize == 1) return this.values[i % this.values.length];
      var ret = [];
    ...
    this.setValue = function (i, v) {
      if (this.direction == VVVV.PinDirection.Output || !this.isConnected()) {
        if (
          !this.typeName ||
          !VVVV.PinTypes[this.typeName].primitive ||
          this.values[i] != v ||
          this.typeName == "Color"
        )
      ...
      if (
        this.node.isIOBox &&
        this.pinname == "Descriptive Name" &&
        this.node.invisiblePins["Descriptive Name"]
      ) {
    ...
    this.connect = function (other_pin) {
      ...
      if (this.direction == VVVV.PinDirection.Output) {
        // this is the case when a subpatch output pin gets connected to the interface pin in the subpatch
    ...
    this.disconnect = function () {
      if (this.typeName == "Color") {
        ...
        this.values = v;
      } else this.values = this.values.slice(0);
      if (this.slavePin) this.slavePin.values = this.values;
      this.markPinAsChanged();
    };
    ...
    this.isConnected = function () {
      return (
        this.links.length > 0 ||
        (this.masterPin && this.masterPin.isConnected()) ||
        (this.slavePin && this.slavePin.links.length > 0)
      );
    };
    ...
    this.setSliceCount = function (len) {
      if (len < 0) len = 0;
      if (this.values.length == len) return;
      if (this.direction == VVVV.PinDirection.Output || !this.isConnected()) {
    ...
    this.setType = function (newType) {
      if (newType.typeName == this.typeName) return;
      var that = this;
      ...
      _(newType.connectionChangedHandlers).each(function (handler, key) {
        that.connectionChangedHandlers[key] =
          newType.connectionChangedHandlers[key];
      });
      this.typeName = newType.typeName;
      this.defaultValue = newType.defaultValue;

      if (
        this.direction == VVVV.PinDirection.Input &&
        this.defaultValue &&
        !this.isConnected()
      ) {
    ...
    if (type == undefined) type = VVVV.PinTypes.Generic;
    if (type == VVVV.PinTypes.Generic) this.unvalidated = true;
    this.setType(type);

    if (init_values && init_values.length > 0) {
      // override PinType's default value with values from constructor, if it isn't []
    ...
    this.generateStaticCode = function (checkForChanges) {
      ...
      nilcode +=
        "false) { patch.nodeMap[" +
        this.node.id +
        "].outputPins['Output'].setSliceCount(0); }\n else {";
      subcode += "0)";
      ...
      subcode += ";\n";
      var code = nilcode;
      if (checkForChanges) code += dirtycode;
      code += "  var iii = ";
      code += subcode;
      code +=
        "  patch.nodeMap[" +
        this.node.id +
        "].outputPins['Output'].setSliceCount(iii);";
      code += "  while (iii--) {\n";
      code +=
        "    patch.nodeMap[" +
        this.node.id +
        "].outputPins['Output'].setValue(iii, " +
        this.values.code +
        ");\n";
      code += "  }\n";
      if (checkForChanges) code += "}\n"; // dirty check
      code += "}\n"; // nil check
      return code;
    };
  };
