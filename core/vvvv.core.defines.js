...
define(function (require, exports) {
  ...
  exports.MousePositions = {
    _all: { x: [0.0], y: [0.0], wheel: [0.0], lb: [0.0], mb: [0.0], rb: [0.0] },
  };
  ...
  exports.Helpers = {
    ...
    prepareFilePath: function (path, patch) {
      ...
      if (path.match(/^%PAGE%/)) {
        // hosting HTML page path
        if (VVVVContext.name === "browser")
          return path.replace("%PAGE%", location.pathname);
        else
          return path.replace(
            "%PAGE%",
            VVVVContext.DocumentRoot + VVVVContext.AppRoot
          );
      }

      if (path.match(/^(\/|.+:\/\/)/)) {
        // path starting with / or an URL protocol (http://, ftp://, ..)
        if (VVVVContext.name === "browser") return path;
        else return VVVVContext.DocumentRoot + path;
      }

      if (patch) path = patch.getAbsolutePath() + path;
