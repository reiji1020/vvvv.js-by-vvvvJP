  ...
  return function (patch) {
    ...
    this.detect = function () {
      ...
      function findCluster(node, clusterActive, nonPrimitiveLink) {
        let fromPin;
        let pinname;
        if (nonPrimitiveLink === undefined) nonPrimitiveLink = false;
        if (node.inCluster) return;
        node.inCluster =
          clusterActive && nonPrimitiveLink && !node.environments;
        ...
        for (pinname in node.inputPins) {
          if (node.inputPins[pinname].links.length > 0) {
            fromPin = node.inputPins[pinname].links[0].fromPin;
            if (!node.delays_output)
              findCluster(
                ...
                node.inCluster ||
                  (clusterActive &&
                    (!node.environments ||
                      (!VVVV.PinTypes[fromPin.typeName].primitive &&
                        node.environments.indexOf("browser") < 0)) &&
                    !node.isSubpatch),
                !VVVV.PinTypes[fromPin.typeName].primitive
              );
            if (
              (clusterActive || !VVVV.PinTypes[fromPin.typeName].primitive) &&
              !node.environments &&
              !node.isSubpatch
            )
        ...
        for (pinname in node.inputPins) {
          if (node.inputPins[pinname].links.length > 0) {
            fromPin = node.inputPins[pinname].links[0].fromPin;
            if (node.inCluster) fromPin.edgeLinkCount--;
        ...
        if (node.inCluster === true) {
          ...
          for (pinname in node.inputPins) {
            node.inputPins[pinname].clusterEdge =
              node.inputPins[pinname].links.length === 0 ||
              !node.inputPins[pinname].links[0].fromPin.node.inCluster;
          }
          ...
          for (pinname in node.outputPins) {
            ...
            node.outputPins[pinname].edgeLinkCount =
              node.outputPins[pinname].links.length;
          }
        }
      }

      for (i = 0; i < patch.nodeList.length; i++) {
        let pinname;
        let n = patch.nodeList[i];
        n.inCluster = false;
        for (pinname in n.inputPins) {
          n.inputPins[pinname].clusterEdge = false;
        }
        for (pinname in n.outputPins) {
          n.outputPins[pinname].clusterEdge = false;
        }
      }
      for (i = 0; i < patch.nodeList.length; i++) {
        if (!patch.nodeList[i].inCluster)
          // the node would have been marked as cluster node if it had already been visited
          findCluster(
            patch.nodeList[i],
            patch.nodeList[i].environments &&
              patch.nodeList[i].environments.indexOf("nodejs") >= 0
          );
      }
      this.clear();
      for (i = 0; i < patch.nodeList.length; i++) {
    ...
    this.addNode = function (node) {
      ...
      if (edgeNode) this.clusterEdgeNodes[node.id] = node;
    };

    this.syncPinValues = function (socket, direction) {
      let nodes = [];
      let edgePins = null;
      if (VVVVContext.name === "browser") edgePins = this.inputEdgePins;
      else edgePins = this.outputEdgePins;
      for (let node_id in edgePins) {
        let pinValues = {};
        let i = edgePins[node_id].length;
        let changedPins = 0;
        while (i--) {
          if (
            edgePins[node_id][i].pinIsChanged() ||
            !edgePins[node_id][i].syncInitialized
          ) {
            if (!VVVV.PinTypes[edgePins[node_id][i].typeName].primitive)
              continue;
            pinValues[edgePins[node_id][i].pinname] =
              edgePins[node_id][i].values;
      ...
      let msg = { patch: patch.getPatchIdentifier(), nodes: nodes };
